function [ssimBA, ssimAB, ssimSym] = ssim_score_jnd(quantA, quantB, idBA, idAB, PARAMS,PossiA,PossiB)
...
%%%     errorBA = zeros(length(PARAMS.ESTIMATOR_TYPE), length(PARAMS.POOLING_TYPE));
    for i = 1:length(PARAMS.ESTIMATOR_TYPE)
        ...
        ssimMapBA = 1 - errorMapBA.*PossiB;                                               % Similarity map as 1 - error_map
...
%%%     errorAB = zeros(length(PARAMS.ESTIMATOR_TYPE), length(PARAMS.POOLING_TYPE));
    for i = 1:length(PARAMS.ESTIMATOR_TYPE)
        ...
        ssimMapAB = 1 - errorMapAB.*PossiA;                                               % Similarity map as 1 - error_map
